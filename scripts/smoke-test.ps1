...
function Invoke-AwsCli([string[]]$Arguments) { & aws @Arguments --endpoint-url $endpoint --cli-connect-timeout 5 --cli-read-timeout 10 }
...
foreach ($required in @('canary-routing-state','canary-releases')) {
    if ($tables -notcontains $required) { throw "Missing DynamoDB table $required" }
}
...
if (-not ($targets | Where-Object { $_.Arn -like '*stateMachine*CanaryReleaseWorkflow*' })) {
    throw 'Rule target is not Step Functions'
}
...
foreach ($required in @('canary-set_weight','canary-evaluate_health','canary-finalize_release')) {
    if ($functions -notcontains $required) { throw "Missing Lambda $required" }
}
if ((Invoke-WebRequest -UseBasicParsing -Uri 'http://localhost:8081/actuator/health').StatusCode -ne 200) {
    throw 'Stable service unhealthy'
}
if ((Invoke-WebRequest -UseBasicParsing -Uri 'http://localhost:8082/actuator/health').StatusCode -ne 200) {
    throw 'Candidate service unhealthy'
}
if ((Invoke-WebRequest -UseBasicParsing -Uri 'http://localhost:8080/actuator/health').StatusCode -ne 200) {
    throw 'Control plane unhealthy'
}
$now = [DateTime]::UtcNow
...
Write-Output 'SMOKE PASS: LocalStack, DynamoDB, EventBridge, Step Functions, Lambda, services, and CloudWatch are available.'
